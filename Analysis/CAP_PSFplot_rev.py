...
for SUBJECT in SUBJECTS:
    for TASK in TASKS:
        ...
        with open(f'{dir_beh}/{SUBJECT}/{TASK_NAME}/log_survey.json', 'r') as f_suv:
            d_suv = json.load(f_suv)['cac']
        f_suv.close()
        
        with open(f'{dir_beh}/{SUBJECT}/{TASK_NAME}/log_gameplay.json', 'r') as f_play:
            d_play = json.load(f_play)['cac']
        f_play.close()
        
        with open(f'{dir_beh}/{SUBJECT}/{TASK_NAME}/log_surrender.json', 'r') as f_sur:
            d_sur = json.load(f_sur)['cac']
        f_sur.close()
        ...
        ax.plot(RHO, PSI_fit_con, linewidth=1, color=COLOR_GOLD_C, zorder=2)
        ...
        raw_diff = np.array(d_play['difficulty'])
        raw_winlose = np.array(d_play['winlose'])
        unique_diff = np.unique(d_play['difficulty'])
        unique_winrate = []
        for diff in unique_diff:
            unique_winrate.append(np.sum(raw_winlose[raw_diff==diff])/np.sum(raw_diff==diff))
        ax.scatter(unique_diff, unique_winrate, s=3, color='red')
        
        
        raw_t = np.array(d_play['time'])
        plot_psi_heatmap(np.round(raw_t/20, 1), np.round(raw_diff, 2), theta_star)
        ...
        suv_ans = np.array([d_suv[suv]['answer'] for suv in d_suv])
        ...
        suv_t = np.array([d_suv[suv]['time'] for suv in d_suv])/20
        ...
        sur_ans = np.array([d_sur[sur]['answer'] for sur in d_sur])
        line1 = ax.plot(np.arange(0,20), sur_ans, color='r', label='vote')
        plt.title(f'{SUBJECT}-{TASK_NAME}-Surrender')
        
        # surrender (t)
        ax2 = ax.twinx()
        sur_t = np.array([d_sur[sur]['time'] for sur in d_sur])/20
        ...
        fig, ax = plot_setting(xlabel='Trials', 
                               xlim=[0, 20], ylim=[0, 100],
                               yticks=[0, 50, 100], yticklabels=[0, 50, 100]) # plot
        suv_ans = np.array([d_suv[suv]['answer'][0] for suv in d_suv])
        line1 = ax.plot(np.arange(0,20), suv_ans, color='b', label='perdiff')
        plt.title(f'{SUBJECT}-{TASK_NAME}-perdiff and realdiff')
        ...
        ax2.set_ylabel('(%)')
        p_win = np.array(d_play['predicted']['winrate'])*100
