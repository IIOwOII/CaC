...
M = 100.0
HRHO = np.array([1.0/RHO, RHO]) # [ses][diff]
...
with open(f'{dir_comp}/pool_psychometric.json', 'r') as f_psy:
    pool_psy = json.load(f_psy)['continuous']['parameter']

# init Param
theta = []
for i in range(4):
    theta.append(np.round(np.linspace(
        pool_psy['min'][i], pool_psy['max'][i]-pool_psy['step'][i], pool_psy['shape'][i]), 3))
theta = np.array(list(itertools.product(*theta)))
theta_idx = np.array(list(itertools.product(*[np.arange(pool_psy['shape'][i]) for i in range(4)])))
GRID_SIZE = np.prod(pool_psy['shape'])

# init P
MU = np.zeros((2, RHO_SIZE, GRID_SIZE))
X_COEF = np.zeros((2, RHO_SIZE, GRID_SIZE))
for ses in range(2):
    MU[ses], X_COEF[ses] = cal_Mu(HRHO[ses], theta) # [ses][diff][grid]
P_con = np.array([cal_Polyexp_PSI(HRHO[0], theta, 'chasing'), 
                  cal_Polyexp_PSI(HRHO[1], theta, 'chased')])
...
for idx, subj in enumerate(SUBJECTS):
    with (open(f'{dir_beh}/{subj}/fitting_chasing/log_fitting.json', 'r') as f0,
          open(f'{dir_beh}/{subj}/fitting_chased/log_fitting.json', 'r') as f1):
        fdat_fit.append([json.load(f0)['cac'], json.load(f1)['cac']])
    with (open(f'{dir_beh}/{subj}/fitting_chasing/log_gameplay.json', 'r') as f0,
          open(f'{dir_beh}/{subj}/fitting_chased/log_gameplay.json', 'r') as f1):
        fdat_play.append([json.load(f0)['cac'], json.load(f1)['cac']])
    with (open(f'{dir_beh}/{subj}/chasing_0/log_survey.json', 'r') as f0,
          open(f'{dir_beh}/{subj}/chased_0/log_survey.json', 'r') as f1):
        dat_suv.append([json.load(f0)['cac'], json.load(f1)['cac']])
    with (open(f'{dir_beh}/{subj}/chasing_0/log_gameplay.json', 'r') as f0,
          open(f'{dir_beh}/{subj}/chased_0/log_gameplay.json', 'r') as f1):
        dat_play.append([json.load(f0)['cac'], json.load(f1)['cac']])
    with (open(f'{dir_beh}/{subj}/chasing_0/log_surrender.json', 'r') as f0,
          open(f'{dir_beh}/{subj}/chased_0/log_surrender.json', 'r') as f1):
        dat_sur.append([json.load(f0)['cac'], json.load(f1)['cac']])
    with (open(f'{dir_beh}/{subj}/chasing_0/log_position.json', 'r') as f0,
          open(f'{dir_beh}/{subj}/chased_0/log_position.json', 'r') as f1):
        dat_pos.append([json.load(f0)['cac'], json.load(f1)['cac']])
