...
public class PrdItemOptionMinusProcedure {
	public static void execute(LevelAccessor world, Entity entity) {
		...
		if ((entity instanceof LivingEntity _livEnt ? _livEnt.getMainHandItem() : ItemStack.EMPTY).getItem() == CacModItems.CAC_TEST_ITEM.get()) {
			...
			if (CacModVariables.MapVariables.get(world).Option_tester < 0) {
				CacModVariables.MapVariables.get(world).Option_tester = CacModVariables.MapVariables.get(world).Option_tester + num_option_tester;
				CacModVariables.MapVariables.get(world).syncData(world);
			}
		}
		if ((entity instanceof LivingEntity _livEnt ? _livEnt.getMainHandItem() : ItemStack.EMPTY).getItem() == CacModItems.CAC_BUILDER_TOOL.get()) {
			CacModVariables.MapVariables.get(world).Option_builder = (CacModVariables.MapVariables.get(world).Option_builder - 1) % num_option_builder;
			CacModVariables.MapVariables.get(world).syncData(world);
			if (CacModVariables.MapVariables.get(world).Option_builder < 0) {
				CacModVariables.MapVariables.get(world).Option_builder = CacModVariables.MapVariables.get(world).Option_builder + num_option_builder;
				CacModVariables.MapVariables.get(world).syncData(world);
			}
		}
	}
