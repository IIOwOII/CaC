...
public class FncManageDifficultyProcedure {
	public static void execute(LevelAccessor world) {
		...
		if ((session).equals("test_mixed")) {
			...
			if (winlose_pre == 1) {
				CacModVariables.Dat_difficulty_absolute = Math.round(Math.pow(10, 2) * (CacModVariables.Dat_difficulty_absolute - 0.05)) / Math.pow(10, 2);
			} else if (winlose_pre == 0) {
				CacModVariables.Dat_difficulty_absolute = Math.round(Math.pow(10, 2) * (CacModVariables.Dat_difficulty_absolute + 0.02)) / Math.pow(10, 2);
			} else {
